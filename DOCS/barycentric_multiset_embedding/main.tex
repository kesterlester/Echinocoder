...
the \emph{barycentric subdivision} of a simplex --- applied \emph{twice}.  The
first two demands will be almost immediate.  The real work, and the heart of
this note, is the second demand, \emph{injectivity}: that distinct multisets
really do land on distinct points.  We give a geometric proof, and explain why a
...
\begin{tikzpicture}[scale=1.5,line join=round]
  ...
  \node[scale=0.8] at (1.15,0.45) {$C_{(1,2,3)}$};
\end{tikzpicture}
...
lies in the cell $C_\sigma$, and its barycentric coordinates there are the
\emph{gaps}
...
corresponding gap $\beta_r$ is \emph{zero}, and $p$ lies on a wall shared by the
two cells that differ only by swapping those two ranks.
\end{proposition}
...
bookkeeping of \emph{which columns} a prefix contains. Record a prefix as a
$k\times n$ matrix --- a \emph{piece} --- that is the indicator row in its own
...
\subsection{Invariance and continuity are immediate}

As promised, two of the three properties cost almost nothing.

\begin{proposition}[(P1) and (P3)]\label{prop:easy}
The map \eqref{eq:output} is permutation-invariant and continuous.
\end{proposition}
...
\]
each step naming the next piece in \emph{gap order}. Thus the entire gap-ordering
of the pieces is recovered, as $(\text{row},\text{size})$ names. Moreover the
...
\subsection{Why two subdivisions, and not one or three}
\label{sec:whytwo}

We can now say precisely why the count is two.

\emph{Not one.} A single subdivision produces only row-local pieces. Forgetting
their labels independently is the product quotient $\Sn^{\,k}$, strictly coarser
than the diagonal $\Sn$ a multiset allows, and Example~\ref{ex:collision}
exhibited two distinct multisets it identifies. One subdivision is provably
insufficient.

\emph{Two suffice.} The second subdivision pools the pieces and forms cumulative
matrices that span all rows. A single such matrix can be canonicalised only by
one common permutation --- the diagonal $\Sn$ --- and (Theorem~\ref{thm:inj}) the
richest one carries the entire column alignment. The over-identification is
repaired.
...
\begin{itemize}[leftmargin=*]
\item One subdivision canonicalises \emph{row-local} objects, which quotients by
      the too-large product group $\Sn^{\,k}$ and collides distinct multisets
      (Example~\ref{ex:collision}).
\item The second subdivision builds \emph{cross-row} matrices; a single such
      matrix can be tidied only by the \emph{diagonal} $\Sn$ a multiset truly
      allows, and --- because each cumulative matrix \emph{remembers its pieces}
      (Lemma~\ref{lem:recover}) and a piece is named label-freely by its
      $(\text{row},\text{size})$ --- the single richest matrix pins the whole
      column alignment (Theorem~\ref{thm:inj}).
\item Two subdivisions are therefore necessary and sufficient: one for each
      level of sorting, the second indispensable precisely because $k\ge2$ rows
      share their columns.
\end{itemize}
