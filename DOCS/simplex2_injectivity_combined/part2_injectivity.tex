...
\subsubsection*{A useful observation: $C\vec U$ and $FC\vec U$ are really sets}
...
\subsubsection{Placing columns into canonical orders -- (a.k.a.~column `sorting')}
...
Here each matrix is tidied \emph{on its own} (\emph{local canonicalisation}, the
embedder's Step~E).  We suppose we have some fixed rule
...
\subsection{The problem we really want to solve}
\label{sec:problem}

Everything in Section~\ref{sec:defs} was scaffolding.  Here is the question we actually
care about.
...
\]
Each is just a list of $k$ sequences of the kind defined in Section~\ref{sec:defs}.  Think of
...
them in the elaborate way Section~\ref{sec:defs} describes?
...
\emph{global} column sorting drawn at the end of Section~\ref{sec:defs}):
\begin{itemize}
...
\subsubsection*{First simplification: the lists are secretly sets}
...
The key was already noticed in Section~\ref{sec:defs}: \emph{a cumulative matrix records its own
...
\label{eq:recovery}
\end{align}
For example, with $C\vec U((3,1),(1,2);3)$ from Section~\ref{sec:defs} the third matrix is
\[
m_3=\begin{bmatrix}1&0&2\\1&1&0\end{bmatrix},
\qquad
\underbrace{\max(1,0,2)}_{2}+\underbrace{\max(1,1,0)}_{1}=3,
...
This section renames the objects of Section~\ref{sec:defs} in a way that makes the proof short.
No new mathematics happens here; we are only choosing better bookkeeping.

\subsubsection*{Slots and slot-sets}

The raw ingredients of Section~\ref{sec:defs} -- the elements of the set
...
\]
The cumulative one-hot vector $coh(\vec w)_m$ from Section~\ref{sec:defs} is precisely the
...
\subsubsection*{The snapshot matrix}

\begin{definition}\label{def:snapshot}
For a configuration $\vec v=(\vec v_1,\ldots,\vec v_k)$ and a slot-set $Q$, the
\emph{snapshot} $S^{\vec v}(Q)$ is the $k\times n$ matrix whose row $j$ is the sum
of the indicators of the prefixes named by $Q$:
\[
\bigl(S^{\vec v}(Q)\bigr)_{\text{row }j}
=\sum_{m\in A_j(Q)} coh(\vec v_j)_m .
\]
\end{definition}

This is nothing more than ``add up the ingredients whose slots lie in $Q$''.  The
link to Section~\ref{sec:defs} is immediate: if $Q_i$ denotes the set of the \emph{first $i$
slots} in an ordering $p$, then $S^{\vec v}(Q_i)$ is exactly the $i$-th cumulative
matrix of $C\vec U(\vec v;p)$.  The word ``snapshot'' is apt: $S^{\vec v}(Q_i)$ is
...
the running example of Section~\ref{sec:defs} -- and the slot-set
...
\[
S^{\vec v}(Q)=\begin{bmatrix}1&0&2\\[1pt]1&1&0\end{bmatrix}.
\]

\subsubsection*{The values depend on $Q$ alone; the configuration only moves them around}
...
\[
\bigl(S^{\vec v}(Q)\bigr)_{j,\,(\vec v_j)_r}
=\#\{\,m\in A_j(Q): m\ge r\,\},
\]
...
\subsection{A snapshot remembers how it was built}
\label{sec:remember}
...
$S^{\vec v}(Q)$ one can recover the set $A$ completely, and the recovery does not
use the sequence $\vec v_j$.  Explicitly, for every integer $t\ge1$,
\[
\#\{\,c:\bigl(S^{\vec v}(Q)\bigr)_{j,c}\ge t\,\}
=
...
The entire slot-set $Q$ can be recovered from the matrix $S^{\vec v}(Q)$: apply
...
\subsection{When are two snapshots the same up to column order?}
\label{sec:align}

Fix a single slot-set $Q$ and compare the two snapshots $S^{\vec v}(Q)$ and
$S^{\vec u}(Q)$ built from it.  By the previous section they have, row by row, the
...
\subsubsection*{Vocabulary: permutations and column equivalence}

\phantomsection\label{def:colequiv}Recall from Section~\ref{sec:defs} that a \emph{permutation} $\sigma\in S_n$ relabels the columns
...
\subsubsection*{Aligning the revealed prefixes}

Recall (Section~\ref{sec:snapshots}) that reading row $j$ of $S^{\vec v}(Q)$ at
...
$S^{\vec v}(Q)\sim S^{\vec u}(Q)$ if and only if $\Sigma(Q)\neq\varnothing$.  More
precisely, $\Sigma(Q)$ is \emph{exactly} the set of column permutations $\sigma$
with $colPerm(S^{\vec v}(Q),\sigma)=S^{\vec u}(Q)$.
\end{lemma}
...
$S^{\vec v}(Q)$ that level set is the prefix $P_{a_t}(\vec v_j)$, with $a_t$ the
...
sends each level set $S$ to $\sigma(S)$.  Hence $colPerm(S^{\vec v}(Q),\sigma)$
equals $S^{\vec u}(Q)$ exactly when, in every row $j$ and at every threshold,
...
\[
S^{\vec u}(Q)=\begin{bmatrix}0&2&1\\1&0&1\end{bmatrix}
=colPerm\!\left(S^{\vec v}(Q),\sigma\right),
\]
so $S^{\vec v}(Q)\sim S^{\vec u}(Q)$ and indeed $\sigma\in\Sigma(Q)$: one
...
\subsubsection*{The richer the snapshot, the fewer alignments survive}
...
\subsubsection*{The retained slot-sets}
...
\[
\{\,canon\,S^{\vec v}(Q):Q\in\mathcal R_{\vec v}\,\}
...
\subsubsection*{Local equality, decoded}
...
\begin{proof}
$localSort$ agreement means the two sets $\{canon\,S^{\vec v}(Q):Q\in\mathcal
R_{\vec v}\}$ and $\{canon\,S^{\vec u}(Q'):Q'\in\mathcal R_{\vec u}\}$ are equal.
By Corollary~\ref{cor:survives}, each matrix $canon\,S^{\vec v}(Q)$ still reveals
...
says $canon\,S^{\vec v}(Q)=canon\,S^{\vec u}(Q)$, i.e.\ $S^{\vec v}(Q)\sim
S^{\vec u}(Q)$.  By Lemma~\ref{lem:align} this is $\Sigma(Q)\neq\varnothing$.  Every
...
\subsubsection*{Global equality, decoded}
...
$S^{\vec v}(Q)$ into $S^{\vec u}(Q)$.  So the whole list of $\vec u$-snapshots is
...
$colPerm(S^{\vec v}(Q),\sigma)=S^{\vec u}(Q)$ for every retained $Q$ says exactly
(Lemma~\ref{lem:align}) that this one $\sigma$ lies in $\bigcap_Q\Sigma(Q)$.
\end{proof}

\subsubsection*{The two conditions coincide}
...
\subsubsection*{The proof in one breath}
...
R_1 & S^{\vec v}(R_1)=\begin{bmatrix}0&0&1&0\\0&0&0&1\end{bmatrix}
    & S^{\vec u}(R_1)=\begin{bmatrix}1&0&0&0\\0&0&1&0\end{bmatrix}\\
R_2 & S^{\vec v}(R_2)=\begin{bmatrix}1&0&2&0\\0&1&0&2\end{bmatrix}
    & S^{\vec u}(R_2)=\begin{bmatrix}2&1&0&0\\0&0&2&1\end{bmatrix}\\
R_3 & S^{\vec v}(R_3)=\begin{bmatrix}2&0&3&1\\1&2&0&3\end{bmatrix}
    & S^{\vec u}(R_3)=\begin{bmatrix}3&2&1&0\\0&1&3&2\end{bmatrix}
\end{array}
\]
...
$colPerm(S^{\vec v}(R_3),\sigma)$ sends the entry in old column $3$ to new column
...
$\left[\begin{smallmatrix}3&2&1&0\\0&1&3&2\end{smallmatrix}\right]=S^{\vec u}(R_3)$.
...
$\{(2,1)\}$,
\[
S^{\vec v}(\{(1,1)\})=\begin{bmatrix}0&0&1&0\\0&0&0&0\end{bmatrix},
\qquad
S^{\vec u}(\{(2,1)\})=\begin{bmatrix}0&0&0&0\\0&0&1&0\end{bmatrix}.
\]
...
\subsubsection*{A second worked example: tied columns, and why a broken tie never returns}
