...
%
% PENDING UNIFICATION (see the reconciliation list handed back with this draft):
% this section currently bridges the TWO ORIGINAL, NOT-YET-UNIFIED notations.
% Most conspicuously, Part I writes the coordinate (row) index as i and the
% vector (column) index as j, whereas the proof writes the coordinate (row)
% index as j and treats columns as symbols/positions.  The dictionary table
% and the boxed caveat below make the correspondence explicit; once a single
% convention is agreed they can be slimmed right down.  No crafted sentence in
% Part I or the proof has been reworded for notation yet.
% ============================================================================
...
one elementary step---and the remainder of the document supplies the proof.
...
The proof that forms the rest of this document rules that out.  Restated in the
notation of Part~I:
...
ordering $p$ of $\vec U$
  ...
  & merge order \\
$C\vec U$, cumulative; $M^{\vec v}(Q)$, snapshot
...
\textbf{One notation seam, not yet sewn.}  Part~I indexes the coordinate (row)
by $i$ and the vector (column) by $j$; the proof below indexes the coordinate
(row) by $j$ and treats the columns as the $n$ \emph{symbols}/positions.  So the
symbol $j$ means a \emph{row} in the proof and a \emph{column} in Part~I---read
the dictionary, not the letters, until the two conventions are unified.
\end{tcolorbox}
...
The remainder of this document makes good on Theorem~\ref{thm:main}.  It
