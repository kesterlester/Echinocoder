...
% ── Misc macros ──────────────────────────────────────────────────────────────
\newcommand{\eji}[2]{e^{#1}_{#2}}           % Eji label: vector index raised, coordinate index lowered
...
\]
to which a small number of \emph{anchor values} (global or per-row extrema)
are appended or prepended to fix the absolute scale.

\subsection*{Why One Number Can Encode Both Coefficients and Basis Elements}

This is the key trick.  The balance has the form $\sum_i \alpha_i L_i$ with all
$\alpha_i \ge 0$: a point in the \emph{positive cone} generated by the $m$
canonical Eji\_LinComb objects $\{L_i\}$.  If each $L_i$ is mapped to a
generic point $p_i \in \R^{N_{\text{hash}}}$, the positive cone of
$\{p_0,\ldots,p_{m-1}\}$ has dimension~$m$.  Two $m$-dimensional cones in
$\R^{2m+1}$ are \emph{generically disjoint} by dimension counting:
$m + m = 2m < 2m+1$, so their intersection has negative expected dimension
and is (with probability~1) empty.  Therefore:

\begin{itemize}[nosep]
  \item A point $p = \sum_i \alpha_i p_i$ in the interior of the cone
        uniquely identifies \emph{which} $\{p_i\}$ were used (i.e.\ which
        canonical $\{L_i\}$, i.e.\ which basis elements).
  \item Once the cone is identified, the $\alpha_i$ are the unique
        non-negative coefficients expressing $p$ in that cone.
\end{itemize}

\noindent
This is why $N_{\text{hash}} = 2m+1$: it is the minimum ambient dimension
for $m$ positive cones to be generically disjoint.

\textbf{Connection to continuity and the simplicial complex.}
When $\alpha_i = 0$ for some~$i$, the point $p$ lies on a \emph{face} of
the cone (a sub-cone with one fewer generator).  Two different cones share
that face if they agree on all basis elements except the one with zero
coefficient.  This is exactly the geometry of a \emph{simplicial complex}:
higher-dimensional simplices glued along lower-dimensional faces.  And these
shared faces are precisely the continuity points discussed earlier: the
transition in sorted order (which changes the basis-element identity) happens
at the same moment as the coefficient goes to zero.  The two events cancel
each other, keeping the output continuous, and the shared face is the
geometric expression of that cancellation.
...
\textbf{Basis-change and continuity picture.}
Algorithm~1 performs the same style of basis change as Algorithm~2
(Section~\ref{sec:alg2}, ``What the Steps Are Actually Doing''), but with one
global sort instead of $k$ per-row sorts.  The global minimum is the coefficient
of the single fully-symmetric basis element $\sum_{i,j}\eji{j}{i}$; the prefix
MSVs $V_r = \{e_0,\ldots,e_r\}$ are the sets of the top-$(r{+}1)$ entries
across \emph{all} rows and vectors combined.  Continuity follows by the same
argument: any gap that goes to zero signals an equality between two globally
adjacent entries, and at that moment the coefficient of the transitioning basis
element is zero.
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Per-row sort.}  For each coordinate $i = 0, \ldots, k{-}1$,
        sort the $n$ values in row~$i$ in \emph{descending} order:
        \[
          X[\sigma_i(0)][i] \;\ge\; X[\sigma_i(1)][i] \;\ge\; \cdots \;\ge\; X[\sigma_i(n-1)][i].
        \]

  \item \textbf{Per-row gaps and prefix MSVs.}  For each row~$i$,
        compute the $n-1$ consecutive differences within that row:
        \[
          \delta^{(i)}_r = X[\sigma_i(r)][i] - X[\sigma_i(r+1)][i], \quad r = 0,\ldots,n-2.
        \]
        Associate gap~$r$ in row~$i$ with the prefix MSV
        $V^{(i)}_r = \{\eji{\sigma_i(0)}{i}, \ldots, \eji{\sigma_i(r)}{i}\}$
        (all entries \emph{within row~$i$}; no cross-row mixing).
        Record the $k$ row minima: $m_i = X[\sigma_i(n-1)][i]$.
  ...
  \item \textbf{Assemble output.}
        \[
          f_2(X) \;=\; \Bigl[\;\underbrace{m_0,\ldots,m_{k-1}}_{k\text{ row minima}},\;
...
\begin{tcolorbox}[notebox]
\textbf{Why no row maxima?}  The maximum of each row (coordinate) is implicitly
encoded: it equals the row minimum plus the sum of that row's gaps
(which are embedded in the hash-weighted sum).  Only the row minima are needed as
anchors to fix the absolute scale.
\end{tcolorbox}

\subsection*{What the Steps Are Actually Doing}

\textbf{Steps 1--2 are a change of basis, not just bookkeeping.}
Write the multiset as $M = \sum_{i,j} x^j_i\, \eji{j}{i}$.
For each row~$i$, the row minimum $m_i$ is the coefficient of the
fully symmetric basis element $\sum_j \eji{j}{i}$, which is invariant under
all permutations of the vectors.  Factoring it out:
\[
  M \;=\; \underbrace{\sum_i m_i \Bigl(\sum_j \eji{j}{i}\Bigr)}_{\text{symmetric part, stored as anchors}}
   \;+\; \underbrace{\sum_{i,r} \delta^{(i)}_r\; V^{(i)}_r}_{\text{balance, all coefficients}\; \ge\, 0}
\]
where the second equality is the Abel (summation-by-parts) identity applied
within each row.  The row minima are recorded directly as anchor values
because they multiply \emph{fully symmetric} basis elements and are
therefore already permutation-invariant.

\textbf{The specific structure of $V^{(i)}_r$ is what matters.}
$V^{(i)}_r = \{\eji{\sigma_i(0)}{i},\ldots,\eji{\sigma_i(r)}{i}\}$ is
not an arbitrary subset of row~$i$: it is the set of the $r{+}1$ vectors that
have the \emph{largest} values in row~$i$, determined by the sorted order $\sigma_i$.
The identity of those vectors (which $j$'s appear) is the non-symmetric information
that cannot yet be discarded.  It is recorded inside the Eji\_LinCombs $L_r$
and ultimately protected by the hash step.

\textbf{Continuity across sorted-order changes.}
The algorithm is $C^0$ even though sorted orders change discontinuously.
The reason: when two values within the same row are equal
($X[\sigma_i(r)][i] = X[\sigma_i(r+1)][i]$), the gap at that position
is exactly~$0$, so the coefficient of the corresponding basis element
$V^{(i)}_r$ is exactly~$0$.  At the very moment the sorted order (and hence
the identity of $V^{(i)}_r$) would switch, we need zero of whichever basis
element we are assigning it to.  The zero coefficient absorbs the
discontinuity in the basis-element identity, keeping the output continuous.
The same argument applies to the global re-sort in Step~3: when two gaps
from different rows are equal, the barycentric coefficient at that rank
is~$0$, so it does not matter which order those two gaps are listed in.
...
\paragraph{Steps 1–2: Per-row sorts and gaps.}
...
\begin{tabular}{clll}
  \toprule
  Row $i$ & Sorted desc.\ (value, label) & Gaps $\delta$ & Prefix MSVs (for each gap) \\
  \midrule
...
\begin{tabular}{clll}
  ...
  \midrule
  0 & 5 & $\{\eji{2}{0}, \eji{0}{0}, \eji{3}{0}\}$ & row~0, gap~2 \\
  1 & 4 & $\{\eji{2}{0}\}$                           & row~0, gap~0 \\
  2 & 3 & $\{\eji{2}{1}, \eji{3}{1}, \eji{1}{1}\}$  & row~1, gap~2 \\
  3 & 2 & $\{\eji{2}{0}, \eji{0}{0}\}$               & row~0, gap~1 \\
  4 & 2 & $\{\eji{2}{1}\}$                            & row~1, gap~0 \\
  5 & 2 & $\{\eji{2}{1}, \eji{3}{1}\}$               & row~1, gap~1 \\
  6 & 1 & $\{\eji{0}{2}\}$                            & row~2, gap~0 \\
  7 & 1 & $\{\eji{0}{2}, \eji{2}{2}, \eji{3}{2}\}$  & row~2, gap~2 \\
  8 & 0 & $\{\eji{0}{2}, \eji{2}{2}\}$               & row~2, gap~1 \\
  \bottomrule
\end{tabular}
\end{center}
...
\[
  f_2(X) = \bigl[\;\underbrace{-3,\;2,\;1}_{3\text{ row min.}},\;
...
\begin{tabular}{p{4.5cm} p{4.5cm} p{4.5cm}}
  ...
  \textbf{Sort strategy} &
    ...
    $k$ independent row sorts \\
  \textbf{Number of gaps} &
    $nk - 1$ &
    $k(n-1) = nk - k$ \\
  \textbf{MSV structure} &
    Prefix sets of the global order; MSVs can mix entries from any row &
    Prefix sets within each row; all entries in a given MSV share the same row ($i$-index) \\
  \textbf{Hash embedding dim.} &
    $2(nk-1)+1 = 2nk-1$ &
    $2k(n-1)+1 = 2nk-2k+1$ \\
  \textbf{Anchor values} &
    Global max and global min (2 values) &
    Per-row minima ($k$ values) \\
...
\begin{enumerate}
  \item \textbf{Fewer gaps to embed ($k{-}1$ fewer).}
        Sorting per-row gives $k(n-1)$ gaps; sorting globally gives $nk-1$
  ...
  \item \textbf{More anchor values ($k{-}2$ more for $k > 2$).}
        ...
        $k$ anchors (per-row minima), which is $k-2$ more for $k > 2$.
...
row}: the smallest entry of each row has no gap below it in that row,
and each such ``missing'' gap is represented by one of the $k$ row-minimum
anchors.  Algorithm~2 exploits the per-row structure to reduce the number of
``missing'' gaps from one (global) to $k$ (one per row), which at first
seems \emph{worse} — but the per-row sorting also prevents $k{-}1$
cross-row comparisons from appearing in the gap list, yielding a net reduction
of $k{-}1$ gaps and ultimately a saving of $k$ output dimensions.

\subsection*{Output Size and the Whitney Embedding Theorem}

Both algorithms produce output of size $\approx 2nk$, which is not obviously
wasteful.  The input space — multisets of $n$ vectors in $\R^k$ — is an
$nk$-dimensional manifold (the symmetric product $SP^n(\R^k)$).  The
\textbf{Whitney Embedding Theorem} states that any smooth $d$-dimensional
manifold can be smoothly embedded into $\R^{2d}$, and that $2d$ is in general
tight.  Output size $\approx 2nk$ is therefore right at the Whitney bound:
anything below $2nk$ would be provably impossible for a smooth embedding of
a generic $nk$-dimensional input space.

The factor of $\approx 2$ is not overhead to be engineered away; it is
intrinsic to what the algorithms are achieving.  Simultaneously encoding
\emph{both} the positive coefficients $\alpha_i$ \emph{and} the identities of
the basis elements $L_i$ inside a single point in $\R^{2m+1}$ (Section
\ref{sec:building-blocks}, ``Why One Number Can Encode Both\ldots'') requires
exactly $2m+1$ dimensions.  This is also the mechanism that delivers
$C^0$ continuity.  The factor of two in the output size is the price of
those properties — and by Whitney it is the minimum such price for a smooth
embedding.

Whether $2nk$ (or $2nk+1$, or $2nk+1-k$) is optimal for the specific
space $SP^n(\R^k)$, or whether some improvement is achievable using
the known structure of that space, remains an open question.
