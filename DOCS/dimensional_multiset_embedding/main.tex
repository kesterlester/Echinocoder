...
\begin{equation}\label{eq:dim}
   d\;=\;1+\operatorname{rank}(R),
\end{equation}
where $R$ is a small table of plain numbers, built as follows. Pick component $0$
as a reference. Then for each \emph{other} component $c=1,\dots,k-1$ write one row
of $R$: the index $c$ itself, followed by the $B$ differences
$\bvec d_c-\bvec d_0$ between that component's unit-exponents and component $0$'s. So
$R$ has $k-1$ rows and $1+B$ columns, and $\operatorname{rank}(R)$ is its
\emph{ordinary matrix rank} --- the number of linearly independent rows (equivalently,
of columns). Then
\[
   \#\text{coefficients}\;=\;O\!\bigl(n^{\,d}\cdot C\bigr),\qquad 2\le d\le B+2,
\]
with $C$ a constant absorbing the small extents ($k$, and the exponent spreads).

It pays to see what each column of $R$ contributes. The first column --- the
indices $1,2,\dots,k-1$ --- is never all zero, so it always supplies one independent
direction; that is the unavoidable ``$+1$'' which, with the $w$ of \eqref{eq:Phi},
gives the base cost $n^2$. The other $B$ columns are the \emph{unit-difference}
columns, and they raise the rank only when the units \emph{actually vary}.
\textbf{If every entry carries one common unit, those columns are identically zero}
and add nothing: $\operatorname{rank}(R)=1$ and $d=2$. A derived unit, however exotic,
costs extra only insofar as it makes these $B$ columns span a new direction --- and
there are only $B$ of them. \textbf{So the cost is set by how the units \emph{vary}
across the $B$ base axes (capped at $B$), never by the sheer number of distinct
derived units on display.}
...
($k=3$, all unit $L$). Because the units are identical, the unit-difference columns
of the table $R$ are entirely zero, leaving $\operatorname{rank}(R)=1$, so
\eqref{eq:dim} predicts $d=2$. We confirm it by direct count. Every
$\boldsymbol\xi^{-\bvec d_i}$ is the same monomial
...
\]
exactly the unit-free cost --- the $d=2$ that $\operatorname{rank}(R)=1$ predicted.
The $\boldsymbol\xi$ contributes no independent direction: with one common unit the
unit-difference columns of $R$ vanish, and the lone $\xi_L$ rides along on $w$ for
free.
\end{example}

\subsection{The dial}

So the whole price is one matrix rank. A few cases, with $R$ built and its rank
read straight off:
...
\begin{tabular}{lccl}
\hline
\textbf{data} & $\operatorname{rank}(R)$ & $d$ & \textbf{cost} \\
\hline
all one unit (e.g.\ positions, any $k$) & $1$ & $2$ & $O(n^2k)$ --- as cheap as unit-free \\
any $k=2$ (so $R$ has just one row) & $1$ & $2$ & $O(n^2k)$ \\
$(\text{mass},\text{length},\text{length})$, $k=3$ & $2$ & $3$ & $O(n^3k)$ \\
units spanning all $B$ axes, one shared & $B{+}1$ & $B{+}2$ & $O(n^{B+2})$ \\
\hline
\end{tabular}
\end{center}

\noindent
(The third row is the running example of Section~\ref{sec:homog}: its two lengths
share a unit, so the index column is independent of the single
mass-versus-length difference, giving $\operatorname{rank}(R)=2$.) The point of the
construction is the cap: a dataset mixing a dozen \emph{derived} units (velocities,
forces, energies, densities, \ldots) built from $B=3$ base ones still has
$\operatorname{rank}(R)\le B+1=4$, so $d\le5$ and the cost is $O(n^{\le5})$ --- no
matter how many derived units appear. They are free riders on the three base axes.
...
\item \textbf{Pay only for base units} (Section~\ref{sec:cost}). The encoding uses
      ...
      $O(n^{\,d})$ with $d=1+\operatorname{rank}(R)\le B+2$ --- the rank measuring how
      the units \emph{vary} across the $B$ base axes, and dropping to the unit-free
      $O(n^2k)$ ($d=2$) whenever the entries are dimensionally uniform.
\end{enumerate}
...
cost of a larger, but still polynomial, code whose exponent is set by how the units
vary across the $B$ base axes. One pays in magnitude-comparisons, the other in the
extra powers of $n$ counted by $\operatorname{rank}(R)$; which is the better bargain
depends on whether the units are genuinely incommensurable.
\end{remark}
...
The method of this note is implemented, and tested, in the \texttt{Echinocoder}
repository (run under its \texttt{venv}). The class is
\texttt{UnitAwareBursarialEncoder} (also exposed as \texttt{Embedder}, the
repository's standard name) in
\begin{center}
\url{https://github.com/kesterlester/Echinocoder/blob/main/Cinf_sympy_unitAwareBursar_embedder_for_array_of_reals_as_multiset.py}
\end{center}
It is a \texttt{MultisetEmbedder} and uses \texttt{sympy} for the polynomial
algebra. It is the unit-aware sibling of the repository's existing (dimensionless)
Bursarial encoders --- the ``not-fair'' \texttt{Cinf\_sympy\_bursar\_\dots} of
Section~\ref{sec:factorise} and the ``fair'' \texttt{Cinf\_sympy\_evenBursar\_\dots}.

\subsection*{What corresponds to what}

\begin{center}
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{@{}p{5.6cm}p{9.0cm}@{}}
\hline
\textbf{This note} & \textbf{Code} \\
\hline
the units, $\bvec d_i$ (one per entry) &
  \texttt{dimensions}, the list of $k$ integer exponent vectors passed to the
  constructor; this fixes $k$ and $B$ \\
homogenised per-vector $\Psi$ \eqref{eq:Psi} &
  \texttt{\_psi}; the per-entry address $t^{i-1}\boldsymbol\xi^{-\bvec d_i}$ is
  \texttt{\_address} \\
multiset polynomial $\Phi$ \eqref{eq:Phi} &
  \texttt{\_phi\_poly}, an expanded \texttt{sympy.Poly} in the gens
  $w,t,\xi_0,\dots,\xi_{B-1}$ \\
the encoding $f$ (coefficients of $\Phi$) &
  \texttt{embed\_generic}; the coefficients are read over a fixed support
  (\texttt{\_canonical\_support}) \\
the encoding size $O(n^{\,1+\operatorname{rank}(R)})$ &
  \texttt{size\_from\_n\_k}; returns $-1$ for any $k$ other than the one fixed at
  construction \\
\hline
\end{tabular}
\end{center}

\subsection*{Three implementation remarks}

\begin{itemize}[leftmargin=*]
\item \textbf{Non-negative exponents.} The code uses $\boldsymbol\xi^{\,\bvec D-\bvec
  d_i}$ in place of $\boldsymbol\xi^{-\bvec d_i}$, where $\bvec D$ is the
  entry-wise maximum of the $\bvec d_i$. As noted in the body, this is one legal
  multiplication by the fixed monomial $\boldsymbol\xi^{\bvec D}$: it gives every
  term the common unit $\text{base}^{\bvec D}$ instead of being dimensionless, so the
  additions remain honest, and it keeps all $\xi$-exponents $\ge0$ so ordinary
  (non-Laurent) polynomials suffice. Injectivity, invariance and continuity are
  untouched.
\item \textbf{Fixed support, so the size is predictable.} The output is the
  coefficient of each monomial in a \emph{data-independent} support --- the
  monomials $\Phi$ carries for generic data, computed once from a symbolic template
  and cached, with the trivial leading $w^n$ term (always $1$) dropped. Coefficients
  that happen to vanish for particular data are still emitted (as $0$). This is why
  the embedding length always equals \texttt{size\_from\_n\_k} exactly, as the
  framework asserts.
\item \textbf{What the test checks.} The accompanying test confirms, on worked
  inputs: permutation-invariance; injectivity (distinct multisets give distinct
  codes, relabelled ones give identical codes, repeated-vector inputs encode
  cleanly); the size matches \texttt{size\_from\_n\_k} and the latter returns $-1$
  for the wrong $k$; the dimensional-covariance law --- each coefficient rescales by
  $\lambda^{(n-a)D_b-m_b}$ under a change of base unit $b$ by $\lambda$
  (Section~\ref{sec:homog}); derived units with negative exponents (velocity,
  energy) work; and the all-positions case collapses to the unit-free Bursar size
  $n+(k-1)n(n+1)/2$, confirming the $d=2$ (i.e.\ $\operatorname{rank}(R)=1$)
  prediction of Section~\ref{sec:cost}.
\end{itemize}
