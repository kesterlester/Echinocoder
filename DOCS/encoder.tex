...
\subsection{The Case Without Sign-Paired Atoms}

For a FlavouredOperator $\mathit{fo} = \fo(\mathit{op}, F)$ without
sign-paired atoms, let $n = \prod_i \comb{n_i}{k_i}$ be the number of
atoms.  Evaluating on event $E$ gives $n$ real numbers
\[
...
\subsection{The Sign-Paired Case: Sign Compression}
\label{sec:5c}

For a $\mathit{fo}$ with sign-paired atoms, the atom set contains pairs
...
Let $n=4$ (labels $a,b,c,d$), one group, and $\mathit{op}=\mathit{dot}$ with
rank~2 and flavour $F=(2)$.  Abbreviate the six atoms as $d_{ab}=\mathit{dot}(a,b)$,
$d_{ac}$, $d_{ad}$, $d_{bc}$, $d_{bd}$, $d_{cd}$.  The valid overlaps for the
self-pairing block $(\mathit{dot},(2),\mathit{dot},(2))$ are $s\in\{0,1,2\}$
since $k_u+k_v-n=2+2-4=0$.
...
\begin{tabular}{c|cccccc}
  \toprule
  & $d_{ab}$ & $d_{ac}$ & $d_{ad}$ & $d_{bc}$ & $d_{bd}$ & $d_{cd}$ \\
  \midrule
  $d_{ab}$ & $\mathbf{2}$ & $1$ & $1$ & $1$ & $1$ & $\underline{0}$ \\
  $d_{ac}$ & $1$ & $\mathbf{2}$ & $1$ & $1$ & $\underline{0}$ & $1$ \\
  $d_{ad}$ & $1$ & $1$ & $\mathbf{2}$ & $\underline{0}$ & $1$ & $1$ \\
  $d_{bc}$ & $1$ & $1$ & $\underline{0}$ & $\mathbf{2}$ & $1$ & $1$ \\
  $d_{bd}$ & $1$ & $\underline{0}$ & $1$ & $1$ & $\mathbf{2}$ & $1$ \\
  $d_{cd}$ & $\underline{0}$ & $1$ & $1$ & $1$ & $1$ & $\mathbf{2}$ \\
  \bottomrule
\end{tabular}
\end{center}
\medskip

Cell entries show the overlap value~$s$.
\textbf{Bold~2} (diagonal): $s=2$, the \texttt{NULL\_SELF} association
(6~cells, always dropped — recoverable from Phase~1 alone).
\underline{Underlined~0} (anti-diagonal): $s=0$, the sole \texttt{ASSOC}
association (6~cells, \emph{encoded}).
Plain~$1$ (all remaining cells): $s=1$, the \texttt{NULL\_COMP} association
(24~cells, dropped — recovered by multiset complement at decode time).
The $6\times6=36$ cells partition as $6+6+24$.

The six encoded pairs are the three ways to partition the four labels
$\{a,b,c,d\}$ into two disjoint pairs of size~2, each appearing twice (once in
each row/column order): $(d_{ab},d_{cd})$, $(d_{cd},d_{ab})$,
$(d_{ac},d_{bd})$, $(d_{bd},d_{ac})$, $(d_{ad},d_{bc})$, $(d_{bc},d_{ad})$.
\end{example}
...
\begin{remark}[Partition identity and sign-partner pairs]
\label{rem:sign-partners}
...
\]
where $\acount(\pf)$ counts \emph{all} sign variants of each label combination
on both sides.

For FlavouredOperators with sign-paired atoms, each label combination yields
both a sign~$+1$ atom ($u^+$) and a sign~$-1$ atom ($u^-$).  The table
therefore contains rows and columns for all sign variants, and the $z$-values
of the sign-partner pairs are algebraically determined by the positive-sign
pairs.  For example, if both operations have sign-paired atoms:
\[
  z(u^+,v^-) = \overline{z(u^+,v^+)}, \quad
  z(u^-,v^+) = -\,\overline{z(u^+,v^+)}, \quad
  z(u^-,v^-) = -z(u^+,v^+).
\]
In all cases the exact relationship is fixed by the orbit type of the pair
(see \cref{sec:5a}).  Sign-partner cells therefore carry no independent
information: encoding the positive-sign pairs suffices, and the sign partners
are recovered automatically using the orbit-type symmetry.  This is why the
encoder restricts its per-association loop to the positive-sign subset
$(u^+, v^+)$ (see \cref{sec:z-embed}).
\end{remark}
...
(bold diagonal cells).  Its six $z$-values are $(1+i)\,d_{xy}(E)$ for each
atom $d_{xy}$ — identical to $(1+i)$ times the Phase~1 orbit for
$\mathit{fo}(\mathit{dot},(2))$.  They are therefore always dropped as
\texttt{NULL\_SELF}, leaving the $s=1$ (24 cells) and $s=0$ (6 cells)
associations.  Of these, $s=1$ is the larger and is dropped as
\texttt{NULL\_COMP}; $s=0$ is the sole encoded association.
\end{example}
...
When the orbit of atom-pairs $(u, v)$ has a non-trivial achievable sign set,
the full orbit of $z$-values has a forced structure that can be exploited to
halve (or quarter) the apparent cost of embedding the full orbit.

The key observation is that the full orbit's $z$-values come in
symmetry-related groups determined by the orbit type:
\begin{itemize}
  \item \textbf{TYPE\_12} (sign flip in $v$ only):
    the full orbit is $\{z_k, \overline{z}_k\}$ — conjugate-closed.
  \item \textbf{TYPE\_21} (sign flip in $u$ only):
    the full orbit is $\{z_k, -\overline{z}_k\}$ — anti-conjugate-closed.
  \item \textbf{TYPE\_22} (independent sign flips in both $u$ and $v$):
...
\subsubsection{TYPE\_11: No sign flips in either atom}
Embed $\{z_k\}$ ($n$ complex roots) directly.  Output: $n$ complex
$= 2n$ reals.  No forced structure.

\subsubsection{TYPE\_12: Sign flip in $v$ only}
...
\subsubsection{TYPE\_21: Sign flip in $u$ only}
...
\subsubsection{TYPE\_22: Independent sign flips in both atoms}
The full orbit is $\{z_k, \overline{z}_k, -z_k, -\overline{z}_k\}$ ($4n$
roots in $n$ quadruples).  Setting $w_k = z_k^2$ reduces to a TYPE\_12-type
...
$= 2n$ reals, after embedding $\{w_k^2\}$ as in TYPE\_12.

\begin{proposition}[Output size]\label{prop:assoc-size}
Every association, regardless of orbit type, contributes exactly
...
factor of~2 or~4 when the orbit has sign-paired atom-pairs).
...
FlavouredOperator, and for FlavouredOperators with sign-paired atoms may flip
the sign of some atoms.  The Phase~1 encoding without sign pairs sorts
evaluation values directly, and sorting commutes with any permutation of the
values.  The sign-paired encoding sorts absolute values, which are additionally
...
possible sign changes, handled by the orbit-type embedding in
...
complementation step (\cref{sec:null-comp}) produces the same dropped-association
...
determines the multiset.  For the sign-paired case, the sorted
...
\begin{description}
  \item[\texttt{ORBIT}] One per distinct FlavouredOperator $\mathit{fo}$ in
    ...
    (\emph{sign-compressed} for FlavouredOperators with sign-paired atoms; label
...
\begin{equation}
  L = \underbrace{\sum_{\mathit{fo} \in \repS} \prod_i \comb{n_i}{k_i}}_{\text{Phase~1}}
...
start index, length, operation names, flavours, overlap, orbit type
(\texttt{TYPE\_11}/\texttt{TYPE\_12}/\texttt{TYPE\_21}/\texttt{TYPE\_22}/\texttt{TYPE\_NEG}),
