...
$m = k(n-1)$ gaps.
Implemented as \texttt{Embedder.embed\_generic} in \algtwopy{} and as
\texttt{simplex\_2\_preprocess\_steps} in \encdecpy{}; see
...
\begin{tcolorbox}[keybox]
\textbf{Source code.}
Both algorithms exist in two independent implementations.

\smallskip
\noindent\textit{Original files} (on which this document is based):
\begin{itemize}[nosep,leftmargin=1.5em,topsep=2pt]
  \item \algtwopy{} — Algorithm~2; entry point \texttt{Embedder.embed\_generic}.
  \item \algonepy{} — Algorithm~1; entry point \texttt{Embedder.embed\_generic}.
\end{itemize}

\smallskip
\noindent\textit{Cleaner reimplementation}:
\begin{itemize}[nosep,leftmargin=1.5em,topsep=2pt]
  \item \encdecpy{} — \texttt{simplex\_2\_preprocess\_steps} (Algorithm~2) and
    \texttt{simplex\_1\_preprocess\_steps} (Algorithm~1) cover Steps~A--E.
    The docstrings use the same $\delta_{(s)}$, $V_{(s)}$, $\Delta_{(s)}$,
    $L_{(s)}$, $\alpha_s$, $\hat{L}_{(s)}$ notation as this document.
  \item \playpy{} — command-line driver for the above.
\end{itemize}

\smallskip
\noindent Step~F (hashing, \texttt{Eji\_LinComb.hash\_to\_point\_in\_unit\_hypercube})
is in the \texttt{C0HomDeg1} files but not yet in \encdecpy{}.
A notation--code correspondence table is in Appendix~\ref{app:code}.
\end{tcolorbox}
...
(scalars  $\Delta_{(s)}$ or $\alpha_{s}$) which are zero by construction ... and so
 the very places where the basis-vectors are ambiguous are also (by construction)
  the very places where those basis vectors do not need to be encoded. Equivalently the
   non-zero values of $\Delta_s$ or $\alpha_s$ all scale basis vectors that are no unambiguously defined.
...
\midrule
\algtwopy & Algorithm~2 original; \texttt{Embedder.embed\_generic}; Steps~A--F \\[4pt]
\algonepy & Algorithm~1 original; \texttt{Embedder.embed\_generic}; Steps~A--F \\[4pt]
\encdecpy & Cleaner reimplementation; \texttt{simplex\_2\_preprocess\_steps} and
...
\begin{sloppypar}
\begin{itemize}[nosep]
\item \textbf{Step~F missing from \encdecpy.}  The \texttt{C0HomDeg1} files
  complete the embedding by hashing each canonical basis matrix to a point in
  the unit hypercube via \texttt{Eji\_LinComb.}\allowbreak
  \texttt{hash\_to\_point\_in\_unit\_hypercube},
  then forming $\sum_s \alpha_s p_s$.  \encdecpy{} stops after Step~E.
