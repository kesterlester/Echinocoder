...
This script is a clone of ``demo_roundtrip.py`` adapted for one of the
"shadow events" discovered by ``symcoder.experiments.encoding_fidelity_search``.
It runs the full encode → decode pipeline **twice** with the same plan:

  1.  on a chosen shadow event E             →  demo_shadow_event_parity_+1.tex
  2.  on its parity flip P(E) = {l: -v}      →  demo_shadow_event_parity_-1.tex

Side-by-side these two PDFs make the parity-blindness of the current
Echinocoder Phase 1 + Phase 2 encoder visible:

  *  The Phase 1 and Phase 2 encoded numerical values are identical in the
     two documents, even though E and P(E) lie in distinct SO(3) orbits
     (their signed scalar-triple products eps3(...) carry opposite sign).
  *  The decoded "alignment table" sections will therefore agree too — the
     decoder is downstream of the encoder, so any information the encoder
     has already lost is unrecoverable by definition.
  *  Only the *event vectors* section at the top differs between the two
     PDFs (and any sections that quote eval(g·atom, event) directly as a
     ground truth, since those are computed from the raw event, not from
     the encoded output).

The plan used here is deliberately limited to ``mag`` + ``euclidean3.dot``
...
This produces both .tex files in one invocation.  See the symcoder
Makefile for the pdflatex rules.
"""
...
# Which shadow event to demonstrate.  Index 0 has clean integer coordinates
# and is generated by the ``random_discrete_011`` family.
SHADOW_INDEX = 0


# ─────────────────────────────────────────────────────────────────────────────
# TeX macro registry (identical to demo_roundtrip.py)
# ─────────────────────────────────────────────────────────────────────────────
...
class DualOut:
    ...
    def write_tex_header(self, title: str, operations=()):
        ...
        self._t(f"\\title{{symcoder parity-blindness demo\\\\\\large {_tex_escape(title)}}}")
        self._t(r"\date{}\begin{document}\maketitle")


# ─────────────────────────────────────────────────────────────────────────────
# TeX helpers (identical to demo_roundtrip.py)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Main demonstration  (one run = one .tex file)
# ─────────────────────────────────────────────────────────────────────────────


def run(event: dict, tex_path: str, parity_sign: int, shadow_index: int):
    """Run the full encode/decode pipeline on ``event`` and write to ``tex_path``.

    ``parity_sign`` is +1 or -1 and is only used in titles / explanatory
    text so that the two output PDFs are visibly labelled.
    """
    assert parity_sign in (+1, -1)
    ...
    with DualOut(tex_path) as out:
        ...
        title_for_pdf = (
            f"shadow event #{shadow_index},  parity = {parity_str}"
        )
        out.write_tex_header(title_for_pdf, operations=plan.operations)

        # ── Banner explaining the parity demo ─────────────────────────────
        ...
        out.kv("This run uses event with parity sign", parity_str,
               tex=rf"\textbf{{This run uses event with parity sign:}} ${parity_str}$\\")
        out.kv("Shadow event index", str(shadow_index),
               tex=rf"\textbf{{Shadow event index:}} ${shadow_index}$ "
                   rf"(from \texttt{{symcoder.tests.\_parity\_shadow\_events.SHADOW\_EVENTS}})\\")
        out.kv("Companion document",
               f"demo_shadow_event_parity_{'-1' if parity_sign > 0 else '+1'}.tex",
               tex=(rf"\textbf{{Companion document:}} \texttt{{demo\_shadow\_event\_parity\_"
                    rf"{'-1' if parity_sign > 0 else '+1'}.tex}}\\"))
        ...
        out.blank()
        out.line(f"Event vectors (3-D)  [parity sign = {parity_str}]:",
                 tex=rf"\textbf{{Event vectors (3-D), parity sign $= {parity_str}$:}}\\")
...
def main():
    ...
    print(f"\n### RUN 1/2: shadow event #{SHADOW_INDEX}, parity sign = +1 ###")
    run(event_pos, TEX_FILE_POS, parity_sign=+1, shadow_index=SHADOW_INDEX)

    print(f"\n### RUN 2/2: shadow event #{SHADOW_INDEX}, parity sign = -1 ###")
    run(event_neg, TEX_FILE_NEG, parity_sign=-1, shadow_index=SHADOW_INDEX)
