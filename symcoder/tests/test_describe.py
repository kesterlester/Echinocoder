...
def test_returns_list_of_segment_info(plan, registry, phase2_factory):
    segs = describe_encoding(plan, registry, phase2_factory)
    assert isinstance(segs, list)
    assert all(isinstance(s, SegmentInfo) for s in segs)

def test_non_empty_for_non_trivial_plan(plan, registry, phase2_factory):
    assert len(describe_encoding(plan, registry, phase2_factory)) > 0

def test_empty_ops_gives_empty_list(ctx, registry, phase2_factory):
    plan = Plan(context=ctx, operations=())
    assert describe_encoding(plan, registry, phase2_factory) == []

def test_all_empty_groups_gives_empty_list(dot, registry, phase2_factory):
...
def test_orbit_segments_present(plan, registry, phase2_factory):
    kinds = {s.kind for s in describe_encoding(plan, registry, phase2_factory)}
    assert "ORBIT" in kinds

def test_assoc_and_null_segments_present(plan, registry, phase2_factory):
    kinds = {s.kind for s in describe_encoding(plan, registry, phase2_factory)}
    assert "ASSOC" in kinds
    assert kinds & set(_NULL_KINDS), f"Expected at least one null kind; got {kinds}"

def test_only_valid_kinds(plan, registry, phase2_factory):
    assert all(s.kind in _ALL_KINDS for s in describe_encoding(plan, registry, phase2_factory))

def test_orbit_segments_come_first(plan, registry, phase2_factory):
    segs = describe_encoding(plan, registry, phase2_factory)
...
def test_null_segments_have_length_zero(plan, registry, phase2_factory):
    assert all(s.length == 0 for s in describe_encoding(plan, registry, phase2_factory) if s.kind in _NULL_KINDS)

def test_null_segments_start_equals_stop(plan, registry, phase2_factory):
    assert all(s.start == s.stop for s in describe_encoding(plan, registry, phase2_factory) if s.kind in _NULL_KINDS)

def test_non_null_segments_are_contiguous(plan, registry, phase2_factory):
    """ORBIT and ASSOC segments must tile the output array without gaps."""
    non_null = [s for s in describe_encoding(plan, registry, phase2_factory) if s.kind not in _NULL_KINDS]
    if not non_null:
        return
...
def test_total_length_matches_encode_output(plan, registry, phase2_factory):
    event = {l: np.random.randn(3) for l in ("a", "b", "c", "d")}
    total = sum(s.length for s in describe_encoding(plan, registry, phase2_factory))
    assert total == len(encode(plan, event, registry, phase2_factory))

def test_stop_property(plan, registry, phase2_factory):
    for s in describe_encoding(plan, registry, phase2_factory):
...
def test_orbit_op_v_is_none(plan, registry, phase2_factory):
    for s in describe_encoding(plan, registry, phase2_factory):
        if s.kind == "ORBIT":
...
def test_orbit_method_name_is_set(plan, registry, phase2_factory):
    """Every ORBIT segment has a non-None method_name from the chosen encoder."""
    for s in describe_encoding(plan, registry, phase2_factory):
        if s.kind == "ORBIT":
            assert s.method_name is not None

def test_orbit_length_symmetric_equals_fo_count(dot, ctx, registry, phase2_factory):
...
def test_orbit_count_for_symmetric_op(dot, ctx, registry, phase2_factory):
    """SYMMETRIC dot with flavour (2,) in group of 4: C(4,2)=6 atoms, length=6."""
    plan = Plan(context=ctx, operations=(dot,))
    orbit_segs = [s for s in describe_encoding(plan, registry, phase2_factory) if s.kind == "ORBIT"]
...
def test_null_self_satisfies_self_pair_condition(plan, registry, phase2_factory):
...
def test_no_assoc_is_self_pair(plan, registry, phase2_factory):
...
def test_each_block_has_at_most_one_null_comp(dot, eps3, ctx, registry, phase2_factory):
    """Each OVERLAP BLOCK has at most one NULL_COMP."""
    segs = describe_encoding(Plan(context=ctx, operations=(dot, eps3)), registry, phase2_factory)
...
def test_block_with_non_self_pair_has_exactly_one_null_comp(dot, eps3, ctx, registry, phase2_factory):
...
def test_null_comp_is_largest_non_self_in_block(dot, eps3, ctx, registry, phase2_factory):
...
def test_symmetry_class_ss_for_dot_dot(dot, ctx, registry, phase2_factory):
    plan = Plan(context=ctx, operations=(dot,))
    assoc_segs = [s for s in describe_encoding(plan, registry, phase2_factory) if s.kind == "ASSOC"]
    assert all(s.symmetry_class == "SS" for s in assoc_segs)

def test_symmetry_class_aa_for_eps3_eps3(eps3, ctx, registry, phase2_factory):
    plan = Plan(context=ctx, operations=(eps3,))
    assoc_segs = [s for s in describe_encoding(plan, registry, phase2_factory) if s.kind == "ASSOC"]
    assert all(s.symmetry_class == "AA" for s in assoc_segs)

def test_symmetry_classes_present_in_mixed_plan(dot, eps3, ctx, registry, phase2_factory):
    plan = Plan(context=ctx, operations=(dot, eps3))
    pair_segs = [s for s in describe_encoding(plan, registry, phase2_factory) if s.kind in _PAIR_KINDS]
...
def test_orbit_str_contains_ORBIT(dot, ctx, registry, phase2_factory):
    plan = Plan(context=ctx, operations=(dot,))
    for s in describe_encoding(plan, registry, phase2_factory):
        if s.kind == "ORBIT":
            assert "ORBIT" in str(s)

def test_assoc_str_contains_op_names(dot, ctx, registry, phase2_factory):
    plan = Plan(context=ctx, operations=(dot,))
    for s in describe_encoding(plan, registry, phase2_factory):
        if s.kind == "ASSOC":
            assert "dot" in str(s)

def test_null_comp_str_contains_null_comp(plan, registry, phase2_factory):
    for s in describe_encoding(plan, registry, phase2_factory):
        if s.kind == "NULL_COMP":
            assert "NULL_COMP" in str(s)

def test_null_self_str_contains_null_self(plan, registry, phase2_factory):
    for s in describe_encoding(plan, registry, phase2_factory):
        if s.kind == "NULL_SELF":
            assert "NULL_SELF" in str(s)

def test_str_contains_index_range(plan, registry, phase2_factory):
    for s in describe_encoding(plan, registry, phase2_factory):
        assert f"[{s.start}:{s.stop}]" in str(s)

def test_assoc_str_contains_symmetry_class(dot, eps3, ctx, registry, phase2_factory):
    plan = Plan(context=ctx, operations=(dot, eps3))
    for s in describe_encoding(plan, registry, phase2_factory):
        if s.kind == "ASSOC":
            assert s.symmetry_class in str(s)

def test_orbit_str_variant_column(dot, eps3, ctx, registry, phase2_factory):
...
def test_all_rows_same_number_of_base_tokens(plan, registry, phase2_factory):
...
def test_full_column_equals_length_for_assoc_and_orbit(plan, registry, phase2_factory):
    """For ASSOC and ORBIT segments, notional_length == length (nothing dropped)."""
    for s in describe_encoding(plan, registry, phase2_factory):
...
def test_full_column_positive_for_null_segments(plan, registry, phase2_factory):
...
def test_str_contains_full_column(plan, registry, phase2_factory):
...
def test_orbit_to_dict_has_required_keys(plan, registry, phase2_factory):
    for s in describe_encoding(plan, registry, phase2_factory):
        if s.kind == "ORBIT":
            d = s.to_dict()
...
def test_assoc_to_dict_has_required_keys(plan, registry, phase2_factory):
...
def test_to_dict_is_json_serialisable(plan, registry, phase2_factory):
    dicts = [s.to_dict() for s in describe_encoding(plan, registry, phase2_factory)]
    dumped = json.dumps(dicts)
    reloaded = json.loads(dumped)
    assert len(reloaded) == len(dicts)
    assert reloaded[0]["start"] == 0

def test_to_dict_start_stop_consistent(plan, registry, phase2_factory):
    for s in describe_encoding(plan, registry, phase2_factory):
        d = s.to_dict()
...
def test_describe_encoding_is_pure(plan, registry, phase2_factory):
    assert describe_encoding(plan, registry, phase2_factory) == describe_encoding(plan, registry, phase2_factory)
