...
* Modify describe.py so that describe_demo would produce something like this:
...
rather than something like this which it does at present:
	...
	...
Probably it would be best to put the decorated operations as an extra column (rather than modifying the first column as I ahve done above) as it's redundant output next to everything earlier on in the output, and nice to keep that output all independent.
