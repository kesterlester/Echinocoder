...
These tests exercise the full machinery — Context, repL,
...
# ---------------------------------------------------------------------------
# repL — FlavouredOperator generation
# ---------------------------------------------------------------------------

def test_repl_with_empty_group_produces_fos(dot, eps3, ctx_one_empty):
    """repL should still produce FOs; empty group contributes 0-count entries."""
...
def test_repl_all_empty_gives_no_fos(dot, ctx_all_empty):
    ...
    fo_list = repL(ctx_all_empty, [dot])
    assert fo_list == []
